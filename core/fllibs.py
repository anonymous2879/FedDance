...
outputClass = {'Mnist': 10, 'cifar10': 10, "imagenet": 1000, 'emnist': 47,
                'openImg': 596, 'google_speech': 35, 'femnist': 62, 'yelp': 5, 'inaturalist' : 1010,
                 "tiny_imagenet": 200, 'fashion_mnist' : 10
                    }  
...
def init_model():
    ...
    if args.task == 'nlp':
        ...
        if args.model == 'LeNet5':    
            from utils.models import LeNetForMNIST 
            model = LeNetForMNIST()
            return model
...
def init_dataset():
    ...
    if args.task == "detection":
        ...
        if args.data_set == 'Mnist':
            ...
            train_transform, test_transform = get_data_transform('tiny_imagenet')
#            train_dataset = datasets.ImageNet(args.data_dir, split='train', download=False, transform=train_transform)   
#            test_dataset = datasets.ImageNet(args.data_dir, split='test', download=False, transform=test_transform)       
            ...
            train_dataset = datasets.FashionMNIST(args.data_dir, train=True, download=False, transform=transforms.Compose([transforms.Resize(size=224), 
            transforms.Grayscale(3), transforms.ToTensor()]))
            test_dataset = datasets.FashionMNIST(args.data_dir, train=False, download=False, transform=transforms.Compose([transforms.Resize(size=224), 
            transforms.Grayscale(3), transforms.ToTensor()]))

        elif args.data_set == 'emnist':
#            test_dataset = datasets.EMNIST(args.data_dir, split='balanced', train=False, download=False, transform=transforms.ToTensor())  
#            train_dataset = datasets.EMNIST(args.data_dir, split='balanced', train=True, download=False, transform=transforms.ToTensor()) 


            test_dataset = datasets.EMNIST(args.data_dir, split='balanced', train=False, download=False, transform=transforms.Compose([transforms.Resize(size=224), 
            transforms.Grayscale(3), transforms.ToTensor()]))
            train_dataset = datasets.EMNIST(args.data_dir, split='balanced', train=True, download=False, transform=transforms.Compose([transforms.Resize(size=224), 
            transforms.Grayscale(3), transforms.ToTensor()]))
