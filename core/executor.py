...
class Executor(object):
    ...
    def __init__(self, args):     
        ...
        self.args = args
        # - update to use the assigned device
    ...
    def init_data(self):
        ...
        # - try to reset the seed to avoid random behaviour before every data loader
        self.setup_seed(self.this_rank)                                      

        training_sets = DataPartitioner(data=train_dataset, numOfClass=self.args.num_class)                     
        # - set the number of clients = to total_clients if clientdatamap file is not present, or total_worker otherwise
    ...
    def run(self):
        try:
            ...
            # - push error message to aggregator and stop
    ...
    def push_msg_to_server(self, event, results):
        ...
        # - to avoid problem of broken pipe
    ...
    # - the model is longer not loaded at every round
    ...
    def training_handler(self, clientId, conf):
        ...
        # load last global model
        # - no need to load the last model, it has been broadcasted from the aggregator
        client_model = self.model #self.load_global_model()



        # global_model_data_loader = select_dataset(self.this_rank, self.testing_sets, batch_size=args.test_bsz, isTest=True, collate_fn=self.collate_fn) 
        global_model_data_loader = select_dataset(clientId, self.training_sets, batch_size=conf.batch_size, collate_fn=self.collate_fn) 
        if self.task == 'voice':
            #criterion = CTCLoss(reduction='mean').to(device=device)
            criterion = torch.nn.CTCLoss(reduction='mean').to(device=self.device)
        else:
            criterion = torch.nn.CrossEntropyLoss().to(device=self.device)  


#        global_model_test_res = test_model(self.this_rank, self.model, global_model_data_loader, device=self.device, criterion=criterion, tokenizer=self.tokenizer)  #对分发的global model在local训练前的测试
#        global_model_test_loss, global_model_acc, global_model_acc_5, global_model_testResults = global_model_test_res
#        self.global_model_acc = global_model_acc
...
#        self.testing_handler(args)
#        train_res['global_model_delta_acc'] = self.global_model_delta_acc
...
#        self.global_model_delta_acc = acc - self.global_model_acc 
