...
class Aggregator(object):
    ...
    def __init__(self, args):
        ...
        # - dynamically set the per round total workers
        self.total_worker = args.total_worker if args.total_worker > 0 else args.initial_total_worker

        #  - used for choosing a replacement client among online if it matches the stale one
    ...
    def tictak_client_tasks(self, sampled_clients, num_clients_to_collect):
        ...
        # - choose random set of clients to drop out based on dropout_ratio
        sampled_dropouts = []
        ...
        for client_to_run in sampled_clients:
            ...
            # - update to obtain the cost from the clients_exe_cost dict of client_manager
            ...
            # - change to dropout the client if he is not online at end of training or he is a dropout candidate
            ...
            # - change to use the full period instead of checking the end of the period
                ...
                #  - account for execution time and communication for online clients
                ...
                #  - account for execution time for dropout clients as they will not communicate
                self.total_compute += exe_cost['computation']

        # - if we do not filter dropouts, we inflate their completion time
        ...
        #  - Change to only top 80% of the clients to simulate failure of 20% with deadline
        if args.deadline_percent > 0:
            num_clients_to_collect = int(math.floor(args.deadline_percent * num_clients_to_collect))

        #  - apply fixed deadline or moving average if it is not zero
        #  - apply deadline for SAFA exp_type=0
        ...
        # - move on to next round and put all clients except for fastest one into  stale cache
        if args.stale_all:
    ...
    def run(self):
        try:
            ...
            # - start wandb run and its communication
            self.start_wandb()
            ...
            #  - add print of the stack call trace to give meaningful debugging information
    ...
    def select_participants(self, select_num_participants, time_window=0):
        # - change to include the deadline for availability calculations
        ...
        #  - handle the case when the requested number is zero
...
#        picked_clients, priority_clients = self.client_manager.resampleClients(select_num_participants, cur_time=self.global_virtual_clock, time_window=time_window)
        ...
        picked_clients, priority_clients = self.client_manager.resampleClients(select_num_participants, cur_time=self.global_virtual_clock, time_window=time_window, cur_epoch = self.epoch)
    ...
    def client_completion_handler(self, results): #, importance=1.0):
        ...
        # - record the number of attended and unique clients
        ...
        # - accumlate train_acc and train_acc_5
        ...
        # - aggregate the label counters from the clients
        self.label_counter.update(results['label_counter'])

        # - Handle and keep a copy of the stale client update
        ...
        # - only update the model if the round did not fail (we have enough new+stale clients)
        if self.round_update and not isStale and not args.send_delta:
            ...
            # - delete the update to free memory
            del results['update_weight']
            self.param_update_count += 1

        #  - invoke the garabage collector to free memory
        gc.collect()

    def save_last_param(self):
        # - maintain the state of the last global model and reset sum of state norms
    ...
    def advance_round(self):
        ...
        #  - change to use the deadline if set
    ...
    def round_completion_handler(self, importance=1.0):    
        ...
        # - change to apply weight updates if round not failed
        if self.round_update:
            self.round_weight_handler(self.last_global_model, [param.data.clone() for param in self.model.parameters()])

            # - Apply the stale updates after the weight handler - to only apply YoGI/Prox to non stale updates only
            # - this did not work out as it leads to divergent behaviour
            #self.stale_clients_handler(importance=importance)

        # TODO:  - this is might be wrong, does not match the paper, in the paper, page 6, the straggler (not ran) clients are not rewarded
        ...
        #  - use the version with availability prioritization
        overcommitment = 1.0
        #  - original oort setting
        ...
        if (self.args.deadline <= 0) or (args.exp_type == 1 or args.exp_type == 3):
            if self.mov_avg_deadline == 0:
                #  - set the initial deadline to something large
        ...
        #  - adapt the number of selected clients but not for SAFA
        ...
        # - SAFA uses all online client for training and the # of workers to wait for is the target ratio
        ...
        # - count the number of times a client is selected
        ...
        logging.info("--------------------------------------------------aggregaotor.py: start loss update------------------------------------------- ")
        for clientId in self.client_manager.feasibleClients: 
            round_loss = 0                                  
            c_a = 0

            num_result = len(self.client_training_results)
            logging.info(f"--------------------------------------------------aggregaotor.py: num of result is {num_result}------------------------------------------- ")
            for i in range(num_result,0,-1):                               
                if num_result == 0:
                    break
#                logging.info(f"--------------------------------------------------aggregaotor.py: i is {i}------------------------------------------- ")
#                logging.info(f"--------------------------------------------------aggregaotor.py: self.client_training_results[{i}] is {self.client_training_results[i]}------------------------------------------- ")
#                logging.info(f"--------------------------------------------------aggregaotor.py: self.client_training_results{i}['clientId'] is {self.client_training_results[i]['clientId']}------------------------------------------- ")
                if (self.client_training_results[i-1]['clientId'] == clientId):  
                    round_loss = self.client_training_results[i-1]['round_loss']
                    c_a = self.client_training_results[i-1]['train_acc']               
#                    c_a = self.client_training_results[i-1]['global_model_delta_acc']  
            
                    break
#            self.client_manager.updateLoss1(clientId, round_loss, self.sampled_participants, self.epoch)

            self.client_manager.updateLoss1(clientId, round_loss, c_a, self.sampled_participants, self.epoch)  
#            self.client_manager.updateLoss2(self.sampled_participants, self.epoch)

            self.client_manager.updateLoss2(self.sampled_participants, self.priority_clients, self.epoch)


        logging.info(f"--------------------------------------------------aggregaotor.py: current round is {self.epoch}------------------------------------------- ")
        ...
        # - changed the way clients are filtered using deadline or deadline_percent
        ...
        # - update the round duration, ensure it is updated before updating the deadline for next round
        self.last_round_duration = self.round_duration
        self.round_duration = round_duration

        # - update the deadline based on the new stragglers
        if (args.exp_type == 0 or args.exp_type == 2):
            # - round duration should be the deadline if not last_worker
        ...
        #  - filter out the clients with pending stale updates if they were selected again
        if self.args.stale_update != 0 or self.args.prohibit_reselect != 0:
            count = 0
            re_sel_client = []
            replacement_clients = []
            # - reselect clients that are stragglers and did not submit an update yet
            ...
            # - reselect clients that were ran less prohibit_select rounds
        ...
        # - ensure unqiue clients in clientstorun
        clientsToRun = list(set(clientsToRun))

        # - record for the last round a client is selected
        for c in clientsToRun:
            self.last_run_round[c] = self.epoch

        #  - round's successful clients
        self.tasks_round = len(clientsToRun)

        if self.tasks_round > 0:
            #  - get the number of stale clients finishing this round
        ...
        # - set the importance of each client finishing in this round
        ...
        # - total updates is the new clients plus the stale ones finishing in this round
        self.total_updates = self.tasks_round + self.round_stale_updates

        # - account for drop out clients if no_filter_dropouts is set
        if self.args.no_filter_dropouts:
            self.total_updates -= len(dropout_clients)

        #  - Perform round update only if the target ratio is met or target ratio is 0 (accept all)
        self.round_update = True

        # - in SAFA if the new clients go to round stragglers if they do not meet the target number
        ...
        #  - extend the clients to run with the strugglers (stale clients) if they are not in the stale state
        #  - include stale clients if the model did not get enough boost last round
        ...
        # - account for compute of straggling clients
        ...
        if self.epoch >= self.args.epochs:
            ...
            # - handle the case when we have 0 clients to run
    ...
    # - define function resetting round metrics
    ...
    def testing_completion_handler(self, results):
        ...
        if len(self.test_result_accumulator) == len(self.executors):
            ...
            if args.use_wandb :
                ...
                # - add more metrics on top 5 test accuracy
                ...
                if (len(self.staleWeights[clientId]) > 0 and clientId in self.staleRemainDuration):
                    if self.staleRemainDuration[clientId] <= duration and self.stale_rounds[clientId] > 0:
                        if self.stale_rounds[clientId] <= args.stale_update or args.stale_update < 0:
                            ...
                            #  - account for stale resources that contributed to the model
    ...
    def stale_clients_handler(self):
        ...
        #  - update the remaining time or delete the stale update if updated already
    ...
    def log_round_metrics(self):
        ...
        #  - process values
        #  - log the train metrics information to file and wandb
        ...
        if args.use_wandb and self.epoch > 1:
            ...
            # - log the fairness of the selection process
    ...
    def normalize_importance(self):
        ...
        if self.round_update:
            for idx, param in enumerate(self.model.parameters()):
                if importance_sum > 0:
                    #  - update the model param by normalizing the importance
    ...
    def update_importance(self):
        ...
        # - update the stale client importance
        ...
        for c in self.round_stale_param:
            #  - just in case a value does not exist
    ...
    def event_monitor(self):
        ...
        while True:
            if len(self.event_queue) != 0:
                ...
                # - handle the case when round failures is disabled and no client meets the deadline
                elif event_msg == 'skip_round':
                    if self.args.stale_skip_round:
                        #  - update the importance of the clients
                        ...
                        #  - invoke the stale clients handler first
                ...
                if event_msg == 'train_nowait':
                    ...
                    #  - perform handler if we have enough number of clients
                    ...
                    if len(self.stats_util_accumulator) == total_clients:
                        ...
                        # - handle the collected deltas from clients
                        ...
                        # - normalize importance and model param
                        self.normalize_importance()
                ...
                # - stop the aggregator if one of the executors report trouble
    ...
    def stop(self):
        ...
        # - terminate after a minute and give wandb time to upload and shutdown wandb
        time.sleep(60)
        # - stop the wandb run
