...
#            os.mkdir(directory, exist_ok=True)  #好像是因为python版本问题，不能使用exist_ok
            os.mkdir(directory)
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            #- only one worker (rank==1) should process the files, make sure the save location is shared by workers
            if args.this_rank == 1:
                ...
                #- other ranks sleep until the main worker (rank=1) finishes and writes the files
                while not os.path.exists(cached_features_file):
                    time.sleep(120)
                #- load the dumped files by main worker (rank 1)
                logger.info("Rank: %s - Loading features from cached file %s", args.this_rank, cached_features_file)
                gc.disable()
                with open(cached_features_file, "rb") as handle:
                    self.examples = pickle.load(handle)
        ...
        #- change to return the actual tokens - does not work, mask tokens expect tensors
